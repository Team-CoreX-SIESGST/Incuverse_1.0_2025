...
import { sendEmailOTP } from "@/services/auth/authServices";
...
export default function AuthStep({ onNext, initialData }) {
  ...
  const handleSendOTP = async (e) => {
    ...
    try {
      const result = await sendEmailOTP({ email: formData.email });
      if (result?.data) {
        setOtpSent(true);
        onNext({
          ...formData,
  ...
  const handleGoogleSuccess = async (response) => {
    ...
    try {
      ...
      onNext({
        ...
        email: formData.email, // You might get this from Google response
      });
    } catch (err) {
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                onChange={handleChange}
                className="block w-full pl-10 pr-10 py-2.5 bg-slate-50/50 dark:bg-slate-800/40 text-slate-900 dark:text-slate-50 rounded-lg ring-1 ring-inset ring-slate-900/10 dark:ring-slate-100/10 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:ring-2 focus:ring-inset focus:ring-emerald-500 transition-all duration-200"
